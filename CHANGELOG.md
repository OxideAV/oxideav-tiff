...
## [0.0.7](https://github.com/OxideAV/oxideav-tiff/compare/v0.0.6...v0.0.7) - 2026-09-01
...
- Tiled `PlanarConfiguration = 2` chroma-subsampled YCbCr, decode **and** encode (TIFF 6.0 §15 + §21 as amended by TIFF Technical Note 2). The TN2 amendment pins the geometry the original §15 text left open: "each strip or tile covers the same image area despite subsampling ... the total number of strips or tiles in the image is the same for each component. Therefore strips or tiles of the subsampled components contain fewer samples" — so `TileOffsets` keeps its `SamplesPerPixel × TilesPerImage` plane-major count, the tile grid comes from the full-resolution dimensions, and a chroma tile holds `TileWidth/sh × TileLength/sv` samples of the reduced §21 "chroma image" (tile dimensions must be integer multiples of the factors). The §14 predictor composes per-plane per-tile. This closes the last supportable subsampled-YCbCr layout combination; the §14 predictor over the packed *chunky* data-unit stream stays rejected (differencing has no defined shape across §21 data-unit boundaries). Validated by cross-layout equality against the strip planar encoding and cross-path equality against the chunky data-unit decoder, over `[2,1]`/`[2,2]`/`[4,2]`, None/LZW/Zstd, ±predictor, and partial edge tiles.
- Opt-in CCITT *uncompressed mode* emission (TIFF 6.0 §11 `T4Options` bit 1 / `T6Options` bit 1; Table 5/T.4 = Table 4/T.6): `TiffCompression::CcittT4TwoD` gains an `uncompressed` flag and `TiffCompression::CcittT6` becomes a struct variant carrying one. With the flag on, every 2-D coded row is encoded both as an ordinary READ row and as an uncompressed segment (entrance code + literal image patterns + exit code) and the cheaper form is emitted — the bit-rate-control role the extension was defined for — and the file advertises the extension via the §11 options bit. Decode support predates this (round trips are byte-exact across solid / run-structured / dithered content, non-byte-aligned widths, multi-strip); finely dithered content gets strictly smaller with the flag on, and run-structured content never regresses. `tiffdump` lists the advertised options bits (libtiff's fax decoder does not implement the extension codes, so pixel-level third-party validation is not available — recorded in the test).
- §22 old-style JPEG **tables-form** decode (`Compression = 6` without an interchange bitstream): the raw §22 table payloads (64-byte zigzag JPEGQTables; JPEGDCTables / JPEGACTables as 16 "BITS" bytes + "VALUES") and each strip's entropy-coded data are wrapped in the ISO 10918-1 marker syntax staged in `docs/image/jpeg/T-REC-T.81-199209-I.pdf` (Annex B: DQT / DHT / DRI / SOF0 / SOF3 / SOS, one synthesized datastream per strip), removing the crate's last recognised-but-rejected §22 layout. Both §22 processes decode — baseline DCT (JPEGProc = 1, SOF0) and lossless Huffman (JPEGProc = 14, SOF3 with the JPEGLosslessPredictors selection-value as Ss and JPEGPointTransforms as Al) — chunky (interleaved, YCbCr sampling factors from `YCbCrSubSampling`) and planar ("one JPEG scan per component", reduced chroma-plane geometry), single- and multi-strip, with table-offset deduplication onto the four T.81 destinations and DRI synthesis from JPEGRestartInterval. Tiled tables-form and per-component predictor / point-transform values that differ within one interleaved scan stay precise errors, as do the established malformed-field messages (in standalone builds too). Validated by decomposing `cjpeg` bitstreams into §22 raw payloads (T.81 marker walk in the test) and requiring byte-identical output against the same bitstream wrapped as `Compression = 7`, plus byte-exact lossless round trips (psv 1/2/4, planar RGB).
- `PlanarConfiguration = 2` JPEG-in-TIFF decode (`Compression = 7`), per TIFF Tech Note 2 "Special considerations for PlanarConfiguration 2": each image segment is a single-channel JPEG datastream, segments run plane-major (`SamplesPerPixel × StripsPerImage` / `× TilesPerImage`), and a subsampled YCbCr chroma segment's SOFn dimensions are scaled down by the sampling factors (last strip rounded up; tile dimensions must be integer multiples of the factors per the TN2-amended §21, as must `RowsPerStrip` of `ChromaSubsampleVert`). RGB (3 planes), YCbCr (full-res Y + reduced Cb/Cr, any §21 factor pair, strip / multi-strip / tiled) and 8-bit CMYK (4 planes, §16 additive conversion) all compose, including the deep 9..=16-bit precisions for RGB / YCbCr. The §22 old-style (`Compression = 6`) interchange layout no longer rejects `PlanarConfiguration = 2` — the complete ISO bitstream declares its own (non-interleaved) scan structure, so the flag changes nothing about the decode. Validated with byte-exact lossless (SOF3) per-plane reassembly and cross-path equality against the crate's independent §21 chunky data-unit decoder.
...
- Independently produced ICC/XMP carriage fixtures committed under `tests/data/icc_xmp/` (classic + BigTIFF files written by a black-box image tool; fully synthetic 132-byte ICC profile + 251-byte XMP packet, so no third-party payload bytes) with SHA-256 + length pins per the trace doc's §5 carriage-fixture shape — `tests/decode_icc_xmp_fixtures.rs` proves byte-exact extraction from files this crate did not write, plus a decode → re-encode → decode preservation loop over the external file. Generation commands staged in `docs/image/tiff/fixtures/icc-xmp/`.
- Black-box interop suite for the ICC/XMP carriage (`tests/icc_xmp_blackbox.rs`, binaries invoked as opaque validator processes, availability-gated): files written by this crate round through `magick` profile extraction byte-exact (classic + BigTIFF, both payloads), survive a `tiffcp -c lzw` rewrite with the ICC profile intact, and list with the intended tag/type/count under `tiffdump`; in the reverse direction a file whose profiles were embedded by `magick` (real system colour profile + XMP packet) decodes to byte-exact payloads.
- Encoder: write the XMP packet (tag 700, BYTE × N via `PageExtras::xmp`) and the embedded ICC profile (tag 34675 InterColorProfile, UNDEFINED × N via `PageExtras::icc_profile`) per `docs/image/tiff/tiff-icc-xmp-tags.md`, interleaved in the §2-required ascending tag order (700 between 532 and 33432; 34675 between the 34665 Exif and 34853 GPS pointers). Payloads are opaque and stored verbatim at even offsets; the ICC profile is validated precisely before writing (fixed 128-byte header present; the profile's own big-endian size field at +0 — the authoritative length — must equal the payload length); an empty XMP packet is rejected. Round-trips byte-exact against the decoder across classic + BigTIFF, single-/multi-strip, tiled, planar, CCITT bilevel and multi-page (distinct payloads per page), including decode → re-encode preservation.
- Decode-side ICC profile + XMP packet extraction (per the trace doc `docs/image/tiff/tiff-icc-xmp-tags.md`): `TiffMetadata` gains `icc_profile` (tag 34675 InterColorProfile — the complete embedded ICC.1 profile, byte-for-byte) and `xmp` (tag 700 — the serialised XMP packet, byte-for-byte). Both field types (BYTE / UNDEFINED) are accepted for either tag; payloads are copied verbatim (never byte-swapped — ICC profiles are internally big-endian regardless of the file's `II`/`MM` order, XMP is UTF-8 text); the inline `Count <= 4` (classic) / `<= 8` (BigTIFF) value-slot path works; the ICC profile's own big-endian size field at offset +0 must equal the IFD `Count` (mismatch = malformed = field dropped without gating the pixel decode). Per page over the multi-page chain via `decode_tiff_all_pages`.

- Encoder: write the remaining TIFF 6.0 §8 ASCII descriptive fields — DocumentName (269), Make (271), Model (272), PageName (285), HostComputer (316) — via new `PageExtras` fields, interleaved into the IFD in the §2-required ascending tag order. The §8 metadata surface is now fully bidirectional: everything the reader exposes, the writer emits, and all ten ASCII fields round-trip.
- Hostile-input hardening for the metadata / format-info extractors: a semantically-malformed informational tag (wrong field type, zero / truncated RATIONAL, out-of-range enum, over-long ASCII count, unterminated / non-UTF-8 ASCII) leaves that one field empty and never panics or gates the pixel decode — proven by a regression suite plus direct-`extract_metadata` unit tests. `decode_tiff_all_pages` and both extractors added to the `decode` fuzz target.
- Structural / codec introspection: `DecodedTiff::format` carries a `TiffFormatInfo` with the raw on-disk layout tags — PhotometricInterpretation, Compression, per-sample BitsPerSample, SamplesPerPixel, PlanarConfiguration, Predictor, FillOrder, SampleFormat, tiled-vs-stripped layout (TileWidth × TileLength or RowsPerStrip) — the values a transcoder / `tiffinfo`-style CLI needs without re-walking the IFD. Total extraction (malformed entries dropped). Also exposed per page over the chain via `decode_tiff_all_pages`.
- Multi-page metadata: `decode_tiff_all_pages` returns one `DecodedTiff` per IFD (the metadata-carrying counterpart of `decode_tiff_all`), so `TiffMetadata` / `TiffFormatInfo` travel with each page's pixels.
